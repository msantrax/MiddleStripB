...
public class GlyphsBuilder {
    ...
    public static GlyphIcon getAwesomeGlyph(GlyphIcons glyph, String color, double scale){
        
        GlyphIcon g = GlyphsBuilder.create(FontAwesomeIconView.class)
                .glyph(glyph)
                //.style(String.format("-fx-fill: %s; -fx-scale-x: %f; -fx-scale-y: %f;", color, scale, scale))
                .style(String.format("-fx-fill: %s;", color))
                .build();
        g.setScaleX(scale);
        g.setScaleY(scale);
    ...
    public static GlyphIcon getMaterialGlyph(GlyphIcons glyph, String color, double scale){
        
        GlyphIcon g = GlyphsBuilder.create(MaterialDesignIconView.class)
                .glyph(glyph)
                .style(String.format("-fx-fill: %s; -fx-scale-x: %f; -fx-scale-y: %f;", color, scale, scale))
                .build();
        return g;
